...
def recurse_build_deps(source_pkg, binaries, sources,
    ...
    recurse_package_deps(source_pkg, binaries, binary_query,
                         hints, pick_first, follow_recommends)
    ...
    for dep in deplist:
        ...
        if binaries[dep].arch == 'src':
            # I don't know why, but sometimes we are getting
            # source RPMs returned here, despite performing a
            # query only against the binary repositories. Might
            # be a bug in recurse_package_deps or python-dnf.
...
sorted. It is recommended to use --hint instead, where practical.
""")
def neededtoselfhost(pkgnames, hint, recommends, merge, full_name, sources,
                     pick_first):
    """
        ...
        if not merge:
            ...
            if sources:
                for key in sorted(sources, key=sources.get):
                    ...
                    if key == pkgname:
                        continue
                    print_package_name(key, sources, full_name)
            else:
                for key in sorted(binaries, key=binaries.get):
                    # Skip the initial package
                    if key == pkgname:
                        continue
                    print_package_name(key, binaries, full_name)
